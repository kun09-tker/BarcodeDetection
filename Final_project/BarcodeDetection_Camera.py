...
def AreaDetection(image):
    ...
    if len(detect_c) != 0:
        i=0
        max = 0
        maxRect = np.max([detect.shape[0] for detect in detect_c])
        for detect in detect_c:
            ...
            if count_children(hierarchy[0],i) >= 40 and detect.shape[0] == maxRect:
